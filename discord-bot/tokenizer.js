...
class Tokenizer {
    ...
    /**
     * Gets all tokens, generating them when necessary
     * @returns {Token[]}
     */
    getTokens() { 
        if (!this.debug && !nostd) {
    ...
    /**
     * Gets the next group from index idx
     * @param {string[]} grouping 
     * @param {number} idx 
     * @returns {[number, Token[]]?} The new value for idx and the group found, or null if none found
     */
    ...
    /**
     * Condenses tokens into groups according to a rule set, so that the tokens are easier to use
     * each item inside the grouping rule follows: (?<type>[?*^])(?<name>.*) where
     *  `?`: Makes the group item optional, such that it isnt required for the group to form
     *  `*`: Makes the group item infinitely repeatable and optional, same as RegExp *
     *  `^`: Makes the group recursive, such that this group can happen inside this same group
     * @param {string[]} grouping 
     * @returns {Token[][]}
     */
    getGroups(grouping) {
